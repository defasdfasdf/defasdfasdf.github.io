...
class Joomla implements AssetsDriver
{
    ...
    protected function makeMinifiedJs($minifiedJsPath, $modifiedDate)
    {
        ...
        foreach (glob(JPATH_SITE . "/media/quix/js/{$this->parent[$modifiedDate]['type']}{$this->parent[$modifiedDate]['id']}-*.js") as $filename) {
            unlink($filename);
        }
    ...
    {
        // remove any previous file for this page
        foreach (glob(JPATH_SITE . "/media/quix/css/{$this->parent[$modifiedDate]['type']}{$this->parent[$modifiedDate]['id']}-*.css") as $filename) {
            unlink($filename);
        }
        ...
        if($cssFileNumber == 1) {
            ...
            $this->putContents($this->cssPath, $this->getContents($this->cssPath) . $this->getBulkCssRules($modifiedDate));

            if ($this->fileExists($this->cssPath)) {
                $this->jFactory->addStyleSheet($minifiedCssPath);
            }
        } else {
            $this->putContents($this->cssPath, $this->getBulkCssRules($modifiedDate));
